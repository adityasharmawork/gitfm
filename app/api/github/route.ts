...
// Replace the fetchGitHubDataPublic function in app/api/github/route.ts
// This version improves commit estimation and data accuracy
...
async function fetchGitHubDataPublic(username: string): Promise<GitHubData> {
  ...
  const [userResponse, reposResponse, contributionData, eventsResponse, starredResponse] = await Promise.all([
    ...
    fetch(
      `${GITHUB_REST_URL}/users/${encodeURIComponent(cleanUsername)}/repos?sort=updated&direction=desc&per_page=100`,
      {
        headers: {
          Accept: "application/vnd.github.v3+json",
          "User-Agent": "CodeSynth-2025",
  ...
  // IMPROVED: Better commit estimation
  // Use contribution data as primary source, with fallback estimation
  const totalContributionsThisYear = contributionData.totalContributions
  
  // Calculate days since account creation to estimate historical activity
  const accountAge = Math.floor((Date.now() - new Date(userData.created_at).getTime()) / (1000 * 60 * 60 * 24))
  const yearsActive = Math.max(1, accountAge / 365)
  
  // Estimate total contributions based on current year's activity
  // If they're active this year, extrapolate; otherwise use a conservative multiplier
  const estimatedTotalContributions = totalContributionsThisYear > 0 
    ? Math.round(totalContributionsThisYear * Math.min(yearsActive, 5)) // Cap at 5 years for conservative estimate
    : Math.round(userData.public_repos * 10) // Fallback: assume ~10 commits per repo
  ...
  const repositories: Repository[] = reposData
    ...
    .map((repo: any) => {
      const userCommitsFromEvents = repoCommitCounts.get(repo.name) || 0
      
      // IMPROVED: Better commit estimation for owned repos
      let commits: number
      if (repo.fork) {
        // For forks, only use actual event commits
        commits = userCommitsFromEvents
      } else if (userCommitsFromEvents > 0) {
        // If we have recent activity, use it
        commits = userCommitsFromEvents
      } else {
        // Estimate based on repo size and age
        const repoAge = Math.floor((Date.now() - new Date(repo.created_at).getTime()) / (1000 * 60 * 60 * 24))
        const repoYears = Math.max(0.1, repoAge / 365)
        
        // Size is in KB, convert to approximate commits
        // Average commit adds ~10-20KB, but this varies widely
        // Use a more realistic multiplier: size / 20 (assumes ~20KB per commit)
        const sizeBasedEstimate = Math.ceil(repo.size / 20)
        
        // Also consider stars and activity as indicators
        const activityMultiplier = repo.stargazers_count > 0 ? 1.5 : 1
        
        // Combine factors for better estimate
        commits = Math.round(sizeBasedEstimate * activityMultiplier)
        
        // Cap at reasonable values based on repo age
        const maxCommitsPerYear = 100
        const maxPossibleCommits = Math.ceil(repoYears * maxCommitsPerYear)
        commits = Math.min(commits, maxPossibleCommits)
        commits = Math.max(commits, 1) // At least 1 commit (initial commit)
      }
  ...
  // IMPROVED: Better total commits calculation
  // Use the sum of repository commits as primary source
  const totalCommitsFromRepos = repositories.reduce((sum, repo) => sum + repo.commits, 0)
  
  // Use the higher of: repo sum, contribution calendar, or estimated total
  const totalCommits = Math.max(
    totalCommitsFromRepos,
  ...
  return {
    ...
    totalStarsGiven: starredCount,
    totalForksEarned,
    totalCommits, // Now uses improved calculation
    longestStreak,
    currentStreak,
    mostActiveDay,
    mostActiveRepo: repositories[0]?.name || null,
...
//       // IMPROVED: If we don't have event data (because it's truncated), calculate a fallback estimate
//       // This prevents "0 commits" on repos the user clearly owns and works on.
//       let commits = userCommits;
      
//       if (userCommits === 0 && !repo.fork) {
//          // Heuristic: If it's your own repo and updated recently, you probably made commits.
//          // We cap this estimate to avoid inflating small repos too much.
//          commits = Math.min(Math.ceil(repo.size / 50), 50); 
//       }
...
//   // FIX: Use the scraped contribution total as the baseline. 
//   // 'commitsFromEvents' is truncated (last 90 days), so it's always too low for active users.
//   // We assume ~90% of a user's total contributions are commits if we don't have exact data.
//   const estimatedTotalCommits = Math.max(
//       commitsFromEvents, 
//       Math.round(contributionData.totalContributions * 0.9)
//   );
...
//     languages,
//     topLanguage: languages[0]?.name || null,
//     totalPRsMerged: prsMerged, // Note: This will still be low (recent only) without OAuth
//     totalPRsOpened: prsOpened,
//     totalIssuesOpened: issuesOpened,
//     totalIssuesClosed: issuesClosed,
//     totalStarsEarned,
